...
import edu.utdallas.seers.lasso.data.entity.constants.*;
...
public class ConstantExtractor extends GenericVisitorWithDefaults<Optional<Constant<?>>, Boolean> {
    private final Logger logger = LoggerFactory.getLogger(ConstantExtractor.class);
    ...
    public Optional<Constant<?>> extractConstant(Expression expression, boolean allowStatic) {
        try {
            ...
            // FIXME deal with these errors
//            logger.error(String.format("Resolution error when trying to find constant: %s, %d",
//                    expression.findCompilationUnit().flatMap(CompilationUnit::getStorage)
//                            .map(storage -> storage.getPath().toString()).orElse("No path"),
//                    expression.getRange().map(r -> r.begin.line).orElse(-1)));
    ...
    public Optional<Constant<?>> visit(EnclosedExpr n, Boolean arg) {
        return extractConstant(n.getInner(), arg);
    }

    @Override
    public Optional<Constant<?>> visit(UnaryExpr n, Boolean arg) {
        UnaryExpr.Operator operator = n.getOperator();
        Optional<Constant<?>> innerResult = extractConstant(n.getExpression(), arg);
